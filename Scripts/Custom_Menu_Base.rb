...
class Game_MenuItem
  ...
  def initialize(symbol, params = {})
    ...
    @refresh  = params[:refresh] || []      # 項目処理後に再描画
  end
  #--------------------------------------------------------------------------
  # ● 
  #--------------------------------------------------------------------------
...
class Scene_Menu
  ...
  def create_background
    create_backimage if CAO::CM::BACKIMAGE && CAO::CM::BACKIMAGE[3]
  ...
  # ● 
  #--------------------------------------------------------------------------
  def create_backimage
    return unless CAO::CM::BACKIMAGE
  ...
  def update_backimage
    ...
    @backimage_sprite.ox -= CAO::CM::BACKIMAGE[1]
    @backimage_sprite.oy -= CAO::CM::BACKIMAGE[2]
  end
  #--------------------------------------------------------------------------
  # ● コマンドウィンドウの作成
  #--------------------------------------------------------------------------
  ...
  #--------------------------------------------------------------------------
  def command_ok
    current_command_window.current_exec
    current_command_window.activate unless current_command_window.locked?
    check_common_event
    check_refresh_window(current_command_window.current_data)
  end
  #--------------------------------------------------------------------------
  # ○ 個人コマンド [決定]
  #--------------------------------------------------------------------------
